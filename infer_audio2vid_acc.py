...
from postprocess import capture_first_frame
...
class EchoMimic_LivePortrait:
    ...
    def parse_args(self):
        ...
        parser.add_argument("-L", type=int, default=2880)
    ...
    def main(self):
        ...
        # set the ref_vid_path and audio_path
        ...
        cmd = f"ffmpeg -i ./silence.wav -i {audio_path} -filter_complex [0:a][1:a]concat=n=2:v=0:a=1[out] -map [out] {new_audio_path}"
        self.run_ffmpeg_command(cmd)
        ...
        ref_vid_name = Path(new_ref_vid_path).stem
        audio_name = Path(new_audio_path).stem
        final_fps = args.fps
        ref_image_path = f"./assets/test_imgs/{ref_vid_name}.png"
        if os.path.exists(ref_image_path):
            os.remove(ref_image_path)
        capture_first_frame(new_ref_vid_path, ref_image_path)
        ...
        save_videos_grid(
            video,
            f"{save_dir}/{ref_vid_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4",
            n_rows=1,
            fps=final_fps
        )
        
        driving_vid_path = f"{save_dir}/{ref_vid_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}.mp4"
        
        # LivePortrait Stage
        portrait_processor = LivePortrait(new_ref_vid_path, driving_vid_path)
        output_vid_path, output_vid_concat_path = portrait_processor.execute_output()
        
        # Generate Background video stage
        scale = self.scale
        position = self.position
        background_path = self.background_path
        video_save_path = self.video_save_path
        foreground_dir = './MODNet/outdir'
        fps = 24
        final = Final_process()
